...
const db_connection =
  "mongodb+srv://admin:admin@cluster1.ujwzkvp.mongodb.net/statement1_student?retryWrites=true&w=majority";
const port = 2324;
...
app.get("/getMoreThan20InDSBDA", async function (request, response) {
  const students = await StudentMarks.find(
    { DSBDA_Marks: { $gt: 20 } },
    { Name: 1 }
  ); // 2nd parameter is for displaying only name
...
app.put("/update10Marks/:studentID", async function (request, response) {
  ...
  const student = await StudentMarks.findOneAndUpdate(
    { _id: studentID },
    {
      $inc: {
        WAD_Marks: 10,
...
app.get("/getMoreThan25InAll", async function (request, response) {
  ...
  let html =
    "The List Of Students Name Who Got More Than 25 Marks In All Subjects:";
...
app.get("/getMoreThan40In2Subjects", async function (request, response) {
  const students = await StudentMarks.find(
    ...
    { Name: 1 }
  ); // 3rd parameter is for displaying only name

  // creating view for browser
  let html =
    "The List Of Students Name Who Got More Than 40 Marks In 2 Subjects:";
  students.map(function (student) {
    html = html + "<li>";
    html = html + student.Name;
    html = html + "</li>";
...
app.delete("/delete/:studentID", async function (request, response) {
  const studentID = request.params.studentID;
  const deletedStudent = await StudentMarks.findOneAndDelete({
    _id: studentID,
  });
